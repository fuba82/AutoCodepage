...
type
  ...
  strict private
    ...
    function    GetFileClassCodePage(Idx: integer): integer;
    procedure   SetFileClassCodePage(Idx: integer; Value: integer);

    function    GetFileClassLanguage(Idx: integer): integer;
    procedure   SetFileClassLanguage(Idx: integer; Value: integer);
  ...
  public
    ...
    procedure   AddFileClass(const AClassName: string; ACodePage, ALanguage: integer);
    procedure   UpdateFileClass(const AClassName, ANewClassName: string; ACodePage, ALanguage: integer);
    ...
    property    FileClassCodePage  [Idx: integer]: integer     read GetFileClassCodePage    write SetFileClassCodePage;
    property    FileClassLanguage  [Idx: integer]: integer     read GetFileClassLanguage    write SetFileClassLanguage;
...
const
  ...
  KEY_CP_NAME:          string = 'Codepage';
  KEY_LANG_NAME:        string = 'Language';
...
// Get codepage/encoding of the file class at a specified array index
function TSettings.GetFileClassCodePage(Idx: integer): integer;
begin
  if not FValid                then exit(0);
  if Idx >= FFileClasses.Count then exit(0);

  Result := FFileClasses[Idx].CodePage;
end;


// Set codepage/encoding of the file class at a specified array index
procedure TSettings.SetFileClassCodePage(Idx: integer; Value: integer);
begin
  if not FValid                then exit;
  if Idx >= FFileClasses.Count then exit;

  FFileClasses[Idx].CodePage := Value;
end;


// Get language of the file class at a specified array index
function TSettings.GetFileClassLanguage(Idx: integer): integer;
begin
  if not FValid                then exit(0);
  if Idx >= FFileClasses.Count then exit(0);

  Result := FFileClasses[Idx].Language;
end;


// Set language of the file class at a specified array index
procedure TSettings.SetFileClassLanguage(Idx: integer; Value: integer);
begin
  if not FValid                then exit;
  if Idx >= FFileClasses.Count then exit;

  FFileClasses[Idx].Language := Value;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        FFileClasses.Last.CodePage := FIniFile.ReadInteger(Groups.Names[GrpCnt], KEY_CP_NAME,   IDM_FORMAT_ANSI);
        FFileClasses.Last.Language := FIniFile.ReadInteger(Groups.Names[GrpCnt], KEY_LANG_NAME, C_NO_LANGUAGE);
      end;
      ...
      begin
        ...
        // Codepage data, if there is any, was already retrieved in the first
        // loop above. So we can delete this key/value pair from the list.
        KeyIdx := GroupExtensions.IndexOfName(KEY_CP_NAME);
        if KeyIdx > -1 then GroupExtensions.Delete(KeyIdx);

        // Language data, if there is any, was already retrieved in the first
        // loop above. So we can delete this key/value pair from the list.
        KeyIdx := GroupExtensions.IndexOfName(KEY_LANG_NAME);
        if KeyIdx > -1 then GroupExtensions.Delete(KeyIdx);
...
begin
  ...
  begin
    ...
    // Write codepage and language data
    FIniFile.WriteInteger(FFileClasses[GrpCnt].Name, KEY_CP_NAME,   FFileClasses[GrpCnt].CodePage);
    FIniFile.WriteInteger(FFileClasses[GrpCnt].Name, KEY_LANG_NAME, FFileClasses[GrpCnt].Language);
...
procedure TSettings.AddFileClass(const AClassName: string; ACodePage, ALanguage: integer);
var
  GrpCnt: integer;
  Idx:    integer;
...
begin
  ...
  FFileClasses[Idx].CodePage := ACodePage;
  FFileClasses[Idx].Language := ALanguage;
...
procedure TSettings.UpdateFileClass(const AClassName, ANewClassName: string; ACodePage, ALanguage: integer);
var
  GrpCnt: integer;
  Idx:    integer;
